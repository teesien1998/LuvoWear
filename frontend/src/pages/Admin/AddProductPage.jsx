...
const AddProductPage = () => {
  ...
  const onDrop = (acceptedFiles) => {
    const newImages = acceptedFiles.map((file) => ({
      url: URL.createObjectURL(file), // Create a URL for the image locally
      altText: file.name,
    }));
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      const uploadedImages = await uploadImages(formData).unwrap();
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 shadow-md rounded-md border">
      <div className="flex space-x-2 items-center justify-center mb-14">
        ...
        <h2 className="text-3xl font-bold text-nowrap">Add Product</h2>
        <p className="w-12 bg-custom h-[2px]"></p>
      </div>
          ...
          <div className="flex gap-8">
            ...
            <div className="flex flex-col gap-4 w-[200px]">
              <Select
                ...
                errorMessage={colorError ? "Please select a color name" : ""}
                variant="bordered"
                label="Color Name"
                labelPlacement="outside"
                ...
                onChange={(e) => {
                  setColorError(false);
                  const value = e.target.value;
                  setSelectedNameColor(value);

                  // Clear input if not Others
                  if (value !== "Others") setOtherNameColor("");
                }}
                classNames={{
                  ...
                  {images.map((image, index) => (
                    <div key={index} className="relative">
                      ...
                      <button
                        ...
                        className="absolute -top-2 -right-2 bg-red-500 font-semibold text-white rounded-full w-5 h-5 text-xs"
                      >
                        ✕
                      </button>
                    </div>
